...
- `external_id`: ID запроса, должен быть уникальным для каждого запроса (в формате UUID V4)
- `threshold`: Значение порога, для сравнения фотографий из камеры и базы данных ГЦП (чем выше показатель порога, тем жестче условия сравнения). Порог можно выставлять 0.5 до 1.0 (0.5 по умолчанию, если отправить параметр пустым).
